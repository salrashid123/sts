...
func validateOptions(opts Options) error {
	...
	if opts.SubjectTokenPath != "" && opts.SubjectTokenSource != nil {
		return errors.New("specify either SubjectTokenPath or SubjectTokenSource ")
	}
...
func constructRequest(ctx context.Context, opts Options) (*http.Request, error) {
	...
	if opts.SubjectTokenPath != "" {
		...
		ts := *opts.SubjectTokenSource
		t, err := ts.Token()
		if err != nil {
			return nil, err
		}
		subToken = []byte(t.AccessToken)
	}
